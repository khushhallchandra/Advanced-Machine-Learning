...
def model(X, w, w2, wn, w3, w4, w_o, p_keep_conv, p_keep_hidden,im_size,phase_train):
    ...
    l1a = tf.nn.sigmoid(batch_norm(tf.nn.conv2d(X, w, strides=[1, 1, 1, 1], padding='SAME'),im_size,phase_train))
                        
    l1 = tf.nn.max_pool(l1a, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
                        
    l1 = tf.nn.dropout(l1, p_keep_conv)

    l2a = tf.nn.sigmoid(batch_norm(tf.nn.conv2d(l1, w2, strides=[1, 1, 1, 1], padding='SAME'),im_size*2,phase_train)) 
                        
    l2 = tf.nn.max_pool(l2a, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME') 
                        
    l2 = tf.nn.dropout(l2, p_keep_conv)

    lna = tf.nn.sigmoid(batch_norm(tf.nn.conv2d(l2, wn, strides=[1, 1, 1, 1], padding='SAME'),im_size*4,phase_train))
    ...
    l3a = tf.nn.sigmoid(batch_norm(tf.nn.conv2d(ln, w3,strides=[1, 1, 1, 1], padding='SAME'),im_size*8,phase_train)) 
    ...
    l4 = tf.nn.sigmoid(tf.matmul(l3, w4))
